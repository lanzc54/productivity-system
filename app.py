...
DEFAULT_ADMIN_CODES = [
    ...
    ("CSCY-NAPP-0000", "Consultancy: Facilitation or Documentation / Review of Procedures / Review of Contracts / Other Tasks with Client"),
    ("TRNG-NAPP-0000", "Training: Probationary Training / Training and Seminars / ITOP Training"),
    ("BMNG-NAPP-0000", "Business Meeting: Quarterly Meeting / Team Meeting / Regional Meeting / Cluster Meeting / Annual Meeting"),
...
def render(content, **context):
    ...
    rendered_content = "<style>.entry-grid select{min-width:190px}.entry-grid .entry-code-input{display:block;width:190px;margin-top:5px}.entry-grid table{min-width:1500px}</style>" + render_template_string(content, **context)
...
def entry_page():
    ...
    content = """<div class='card'><h2>Time entry</h2>{% if role == 'admin' %}<form method='get'><input type='hidden' name='tab' value='entry'><label>Auditor<select name='auditor' onchange='this.form.submit()'><option value=''>Select auditor account</option>{% for account in auditor_accounts %}<option value='{{account.auditor}}' {% if account.auditor==selected %}selected{% endif %}>{{account.auditor}} ({{account.username}})</option>{% endfor %}</select></label></form>{% endif %}<p class='muted'>Each hour counts as 0.125 MD. Overtime codes are available for registered engagements. Type a code or choose one from the dropdown.</p><datalist id='entry-code-suggestions'>{% for code in entry_codes %}<option value='{{code}}'></option>{% endfor %}</datalist><div class='grid entry-grid'><table><tr><th>Date</th>{% for slot in slots %}<th>{{slot}}</th>{% endfor %}</tr>{% for day in days %}<tr><td><b>{{day.strftime('%a')}}</b><br>{{day.isoformat()}}</td>{% for slot in slots %}<td><form method='post' action='{{url_for("save_entry")}}'><input type='hidden' name='csrf_token' value='{{ csrf_token }}'><input type='hidden' name='auditor' value='{{selected}}'><input type='hidden' name='work_date' value='{{day.isoformat()}}'><input type='hidden' name='slot' value='{{slot}}'><select name='code' onchange='this.form.querySelector("[name=typed_code]").value=this.value;this.form.submit()' title='{{values.get((day.isoformat(),slot), "")}}'><option value=''>-</option>{% for code in entry_codes %}<option value='{{code}}' {% if values.get((day.isoformat(),slot))==code %}selected{% endif %}>{{code}}</option>{% endfor %}</select><input class='entry-code-input' name='typed_code' list='entry-code-suggestions' value='{{values.get((day.isoformat(),slot), "")}}' placeholder='Type code' onchange='this.form.submit()' aria-label='Type engagement code'></form></td>{% endfor %}</tr>{% endfor %}</table></div></div>"""
...
def save_entry():
    ...
    code = request.form.get("typed_code", "").strip() or request.form.get("code", "").strip()
...
def codes_page(tab):
    ...
    admin_rows = "".join(f"<tr><td>{escape(row['code'])}</td><td>{escape(row['description'])}</td><td><form method='post' action='{url_for('delete_code_by_details')}'>{csrf_field()}<input type='hidden' name='kind' value='admin'><input type='hidden' name='code' value='{escape(row['code'])}'><input type='hidden' name='description' value='{escape(row['description'])}'><button class='btn danger'>Delete</button></form></td></tr>" for row in rows) if kind == "admin" else ""
    admin_table = f"<section><h3>Non-engagement code table</h3><table><tr><th>Code</th><th>Description / particulars</th><th>Actions</th></tr>{admin_rows or '<tr><td colspan=3>No non-engagement codes recorded</td></tr>'}</table></section>" if kind == "admin" else ""
    catalog_sections = ""
    if kind == "engagement":
